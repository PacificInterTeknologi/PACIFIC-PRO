...
function updateLaporanKeuanganOnPayment(penjualan) {
  try {
    ...
    if (penjualan.status === "Lunas" && penjualan.tanggalPelunasan) {
      // Validasi format tanggal pelunasan
      if (!isValidDate(penjualan.tanggalPelunasan)) {
        throw new Error("Format tanggal pelunasan tidak valid");
      }
      
      // Hapus dulu data laporan keuangan untuk pelunasan invoice ini jika ada (untuk menghindari duplikat)
      const filteredLaporan = laporanKeuangan.filter(item => 
        !(item.noInvoice === penjualan.noInvoice && item.jenisTransaksi === "pelunasan")
      );
    ...
    showNotification("Terjadi kesalahan saat memperbarui laporan keuangan: " + error.message, "error");
  }
}
/**
  ...
  try {
    ...
    if (status === "Lunas") {
      ...
      if (!invoice.tanggalPelunasan) {
        ...
        invoice.tanggalPelunasan = today.toISOString().split('T')[0];
      } else {
        // Validasi format tanggal pelunasan yang sudah ada
        if (!isValidDate(invoice.tanggalPelunasan)) {
          showNotification("Format tanggal pelunasan tidak valid. Menggunakan tanggal hari ini.", "warning");
          const today = new Date();
          invoice.tanggalPelunasan = today.toISOString().split('T')[0];
        }
      }
      ...
      if (oldStatus === "Belum Lunas") {
        ...
        jurnalData.push({
          tanggal: invoice.tanggalPelunasan,
          akun: "Piutang Usaha",
    ...
    showNotification("Terjadi kesalahan saat mengubah status pembayaran: " + error.message, "error");
  }
}
/**
 ...
 * @param {string} chartName - Nama variabel global chart
 */
function safeDestroyChart(chartName) {
  try {
    if (window[chartName] && 
        typeof window[chartName].destroy === 'function') {
...
function buatChartPendapatan(canvasId, periode = 'bulan') {
  try {
    ...
    const dataPendapatan = laporanKeuangan.filter(item => 
      item.akun === "Penjualan" && item.tipe === "kredit" && isValidDate(item.tanggal)
    );
...
function buatChartPerbandingan(canvasId) {
  try {
    ...
    const dataPelunasan = laporanKeuangan.filter(item => 
      item.akun === "Pelunasan" && item.tipe === "debit" && isValidDate(item.tanggal)
    );
    ...
    // Hancurkan chart yang sudah ada jika ada dengan aman
    safeDestroyChart('comparisonChart');
...
function initializeApp() {
  ...
  switch(pageName) {
    ...
    case 'dashboard.html':
      ...
      }, 500); // Tunggu 500ms untuk memastikan elemen canvas sudah dimuat
      break;
